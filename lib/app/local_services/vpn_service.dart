...
import 'package:skivpn/app/utils/app_args.dart';
...
class VPNService {
  ...
  static init() async {
    ...
    if (currentSession != null && currentSession.subscribeUrl.isNotEmpty) {
      final profile = ProfileManager.getByUrl(currentSession.subscribeUrl);
      if (profile != null) {
  ...
  static Future<bool> _prepareConfig(ProfileSetting profile) async {
    ...
    await ClashSettingManager.saveCorePatchFinal(
      proxyGroups.groups,
      proxyGroups.selectGroupName,
      profile.overwriteDns,
    );
  ...
  static Future<ReturnResultError?> restart(Duration timeout) async {
    ...
    if (currentSession == null || currentSession.subscribeUrl.isEmpty) {
      return ReturnResultError(
        "current session is empty or subscribeUrl is empty",
      );
    }
