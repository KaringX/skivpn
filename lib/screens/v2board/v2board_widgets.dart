...
class V2boardWidgetPlan extends StatefulWidget {
  final BoardSession session;
  const V2boardWidgetPlan({super.key, required this.session});

  @override
  State<V2boardWidgetPlan> createState() => _V2boardWidgetPlanState();
}

class _V2boardWidgetPlanState extends State<V2boardWidgetPlan> {
  bool _loading = false;
  ...
  Future<void> _refreshPlan() async {
    ...
    if (session?.authData.isNotEmpty ?? false) {
      _loading = true;
      V2boardLogin.getSubscribe(session!, reloadProfile: false).then((value) {
        if (!mounted) {
          return;
        }
  ...
  void _refreshSubscribeInfo(BoardSession session) async {
    ...
    String? err = await V2boardLogin.getSubscribe(session);
    if (!mounted) {
      return;
    }
    _lastUpdate = DateTime.now();
